...
struct ComponentContextTests {
    ...
    func fromComponent() {
        ...
        let ctx = ComponentContext.from(
            component: component,
            dataContext: DataContext(dataModel: DataModel())
        )
        #expect(ctx?.componentId == "btn1")
        #expect(ctx?.componentType == "Button")
        #expect(ctx?.properties["label"] == .string("Submit"))
        #expect(ctx?.properties["id"] == nil)
        #expect(ctx?.properties["component"] == nil)
    }
    ...
    @Test("dispatch forwards action name and context")
    func dispatchForwards() {
        ...
        let cc = ComponentContext(
            ...
            properties: [:],
            dataContext: DataContext(dataModel: DataModel()),
            dispatch: { name, context in box.name = name; box.ctx = context }
        )
        cc.dispatch("submit", ["formId": .string("f1")])
        #expect(box.name == "submit")
        #expect(box.ctx?["formId"] == .string("f1"))
    }
}
