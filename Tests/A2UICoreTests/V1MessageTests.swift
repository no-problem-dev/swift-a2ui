...
// MARK: - v1.0 official test-case corpus (golden)

/// Decodes the official `specification/v1_0/test/cases/*.json` corpus through the Swift wire types.
///
...
@Suite("v1.0 official test-case corpus")
struct V1CorpusTests {
    ...
    @Test("all valid agent→renderer messages decode and round-trip", arguments: [
        "case_call_function_message", "case_initial_state_validation",
    ])
    func agentMessages(_ file: String) throws {
        let cf = try load(file)
        #expect(cf.schema == "agent_to_renderer.json")
        for c in cf.cases where c.valid {
            let decoded = try JSONDecoder().decode(AgentMessage.self, from: c.data)
            let reencoded = try JSONEncoder().encode(decoded)
            let redecoded = try JSONDecoder().decode(AgentMessage.self, from: reencoded)
            #expect(decoded == redecoded, "round-trip mismatch: \(c.description)")
        }
    }

    @Test("all valid renderer→agent messages decode and round-trip", arguments: [
        "case_function_response", "case_renderer_messages",
    ])
    func rendererMessages(_ file: String) throws {
        let cf = try load(file)
        #expect(cf.schema == "renderer_to_agent.json")
        for c in cf.cases where c.valid {
            let decoded = try JSONDecoder().decode(RendererMessage.self, from: c.data)
            let reencoded = try JSONEncoder().encode(decoded)
            let redecoded = try JSONDecoder().decode(RendererMessage.self, from: reencoded)
            #expect(decoded == redecoded, "round-trip mismatch: \(c.description)")
        }
    }
    ...
    func structuralFailures() throws {
        for file in ["case_call_function_message", "case_function_response"] {
            let cf = try load(file)
            let isAgent = cf.schema == "agent_to_renderer.json"
            for c in cf.cases where !c.valid && hasStructuralOmission(c.data) {
                #expect(throws: (any Error).self, "should reject: \(c.description)") {
                    if isAgent { _ = try JSONDecoder().decode(AgentMessage.self, from: c.data) }
                    else { _ = try JSONDecoder().decode(RendererMessage.self, from: c.data) }
                }
            }
        }
    }
...
// MARK: - v1.0 new message types (focused)

@Suite("v1.0 CallFunctionMessage / FunctionResponse")
struct V1FunctionMessageTests {
    ...
    @Test func functionResponseRoundTrip() throws {
        let msg: RendererMessage = .functionResponse(FunctionResponse(
...
@Suite("v1.0 ActionResponseMessage")
struct V1ActionResponseTests {

    @Test func valueResponseRoundTrip() throws {
        let msg: AgentMessage = .actionResponse(ActionResponseMessage(
    ...
    @Test func errorResponseRoundTrip() throws {
        ...
        let data = try JSONEncoder().encode(AgentMessage.actionResponse(arm))
        let decoded = try JSONDecoder().decode(AgentMessage.self, from: data)
...
// MARK: - v1.0 type additions

@Suite("v1.0 type additions")
struct V1TypeAdditionTests {
    ...
    @Test func userActionWantResponseAndActionId() throws {
        ...
        let data = try JSONEncoder().encode(RendererMessage.action(ua))
        let decoded = try JSONDecoder().decode(RendererMessage.self, from: data)
    ...
    @Test func clientErrorCorrelatesToFunctionCall() throws {
        let json = #"{"version":"v1.0","error":{"code":"FUNCTION_FAILED","functionCallId":"call-9","message":"boom"}}"#
        let decoded = try JSONDecoder().decode(RendererMessage.self, from: Data(json.utf8))
